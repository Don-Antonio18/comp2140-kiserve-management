...
function getSelectedStatuses() {
    return Array.from(includeStatusesSelect.selectedOptions).map(opt => opt.value);
}
...
function buildReportTemplate(filteredProjects, meta) {
    ...
    const ongoing = filteredProjects.filter(p => p.status === "active" || p.status === "pending");
...
`;
    ...
    const ongoingLines = ongoing.length === 0
   ...
   Committee: ${p.committee || "N/A"}
   Location: ${p.location || "N/A"}
   Hours so far: planning ${p.hoursPlanning ?? 0}h, executing ${p.hoursExecuting ?? 0}h
...
function handleGenerateReport(event) {
    ...
    projectsSummarySpan.textContent =
        `${filtered.length} project(s) included in this report out of ${allProjects.length} total.`;
}
