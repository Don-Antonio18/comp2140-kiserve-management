...
function renderProjects() {
    ...
    projects.forEach((project, index) => {
        ...
        const related = allMilestones.filter(m => String(m.projectId) === String(project.id));
        const totalMilestones = related.length;
        const doneMilestones = related.filter(m => m.completed).length;
        const milestoneSummaryHTML = totalMilestones > 0 ? `<div class="milestone-summary" style="font-size:12px;color:#555;margin-top:6px">${doneMilestones}/${totalMilestones} milestones complete</div>` : '';
...
function addProject(event) {
    ...
    if (editingIndex !== null && projects[editingIndex]) {
        ...
        existing.hoursPlanning = hoursPlanningInput.value ? Number(hoursPlanningInput.value) : null;
        existing.hoursExecuting = hoursExecutingInput.value ? Number(hoursExecutingInput.value) : null;
        existing.beneficiaries = beneficiariesInput.value.trim();
        existing.peopleImpacted = peopleImpactedInput.value ? Number(peopleImpactedInput.value) : null;
...
if (loadSampleBtn) {
    loadSampleBtn.addEventListener('click', () => {
        if (!confirm('This will insert sample projects into storage. Continue?')) return;
